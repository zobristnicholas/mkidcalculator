...
def noise_monte_carlo(n_samples, f, t_hot, t_cold, s0, s1, s2=None, s3=None,
    ...
    Args:
        ...
        t_cold: float
            ...
            attenuator.
        s0: numpy.ndarray
            Average of the single-sided PSD_II and PSD_QQ when the HEMT input
            is terminated at the t_hot attenuator.
        s1: numpy.ndarray
            Average of the single-sided  PSD_II and PSD_QQ when the HEMT
            input is terminated at the t_cold attenuator.
        s2: numpy.ndarray (optional)
            Average of the single-sided PSD_II and PSD_QQ when the HEMT
            input is connected to an MKID. If an MKID is connected to the
            same switch as the calibration attenuators, this noise spectrum
            can be provided to calculate the input noise. The default is
            None, which assumes no MKID is connected.
        s3: numpy.ndarray (optional)
            Average of the single-sided PSD_II and PSD_QQ when the HEMT input
            is connected to an MKID and a parametric amplifier is turned on. If
            a parametric amplifier (or other amplifier that has unity gain
            when off) is between the MKID and the same switch as the
            calibration attenuators, this noise spectrum can be provided to
            calculate the parametric amplifier noise. The default is None,
...
def compute_noise_numbers(f, t_hot, t_cold, s0, s1, s2=None, s3=None,
    ...
    s_hot = sc.h * f * z0 / np.tanh(sc.h * f / (2 * sc.k * t_hot)) / 4
    s_cold = sc.h * f * z0 / np.tanh(sc.h * f / (2 * sc.k * t_cold)) / 4
    sz = sc.h * f * z0 / 4
    # HEMT gain and noise
    ...
    if s2 is not None:
        ...
        ah_sys = 2 * s2 / (sc.h * f * z0 * gh_sys)  # total system noise
